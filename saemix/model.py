...
class SaemixModel:
    ...
    def _validate_model(self):
        ...
        if len(params) != 3:
            raise ValueError("model function must accept exactly 3 arguments: psi, id, xidep")
    ...
    def _process_psi0(self, psi0, name_modpar):
        ...
        if name_modpar is None:
            if hasattr(psi0_arr, 'columns') or (hasattr(psi0, 'columns') if not isinstance(psi0, np.ndarray) else False):
                name_modpar = list(psi0_arr.columns) if hasattr(psi0_arr, 'columns') else list(psi0.columns)
            else:
    ...
    def _validate_sizes(self):
        ...
        if len(self.transform_par) != npar:
            raise ValueError(f"transform_par length ({len(self.transform_par)}) must match number of parameters ({npar})")
        
        if len(self.fixed_estim) != npar:
            raise ValueError(f"fixed_estim length ({len(self.fixed_estim)}) must match number of parameters ({npar})")
        
        if self.covariate_model.shape[1] != npar:
            raise ValueError(f"covariate_model columns ({self.covariate_model.shape[1]}) must match number of parameters ({npar})")
        ...
        if self.covariance_model.shape[0] != npar:
            raise ValueError(f"covariance_model size ({self.covariance_model.shape[0]}) must match number of parameters ({npar})")
        ...
        if self.omega_init.shape[0] != npar:
            raise ValueError(f"omega_init size ({self.omega_init.shape[0]}) must match number of parameters ({npar})")
        ...
        for em in self.error_model:
            if em not in valid_models:
                raise ValueError(f"Invalid error model: {em}. Must be one of {valid_models}")
        
        if self.modeltype not in ['structural', 'likelihood']:
            raise ValueError(f"Invalid modeltype: {self.modeltype}. Must be 'structural' or 'likelihood'")
