import numpy as np
from saemix.utils import transphi, cutoff, mydiag
...
def fim_saemix(saemix_object):
    ...
    try:
        ...
        se_res = np.sqrt(np.diag(cov_fim[npar+nomega:])) if nres > 0 else np.array([])
    ...
    if saemix_object.options.get('warnings', True):
        print("Note: This is a simplified FIM implementation. For full implementation, refer to R code.")
