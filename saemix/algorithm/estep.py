...
from saemix.algorithm.map_estimation import error_function, conditional_distribution_c, conditional_distribution_d
...
def estep(kiter, Uargs, Dargs, opt, mean_phi, varList, phiM):
    nb_etas = len(varList['ind_eta'])
    omega_eta = varList['omega'][np.ix_(varList['ind_eta'], varList['ind_eta'])] if nb_etas > 0 else np.zeros((0, 0))
    domega = cutoff(np.diag(omega_eta), 1e-10) if nb_etas > 0 else np.array([])
    omega_eta = omega_eta - np.diag(np.diag(omega_eta)) + np.diag(domega) if nb_etas > 0 else omega_eta
    ...
    etaM = phiM[:, varList['ind_eta']] - mean_phiM[:, varList['ind_eta']] if nb_etas > 0 else np.zeros((phiM.shape[0], 0))
    phiMc = phiM.copy()
    ...
    if nb_etas > 0 and nbiter_mcmc[0] > 0:
        for _ in range(nbiter_mcmc[0]):
            etaMc = np.random.randn(Dargs['NM'], nb_etas) @ chol_omega.T
            phiMc[:, varList['ind_eta']] = mean_phiM[:, varList['ind_eta']] + etaMc
            Uc_y, _ = compute_LLy(phiMc, Dargs, varList['pres'])
            deltau = Uc_y - U_y
            ind = np.where(deltau < -np.log(np.random.rand(Dargs['NM'])))[0]
            etaM[ind, :] = etaMc[ind, :]
            U_y[ind] = Uc_y[ind]
            phiM[ind[:, None], varList['ind_eta']] = mean_phiM[ind[:, None], varList['ind_eta']] + etaM[ind, :]

    # Kernel 2: component-wise random walk (nrs2 = 1)
    if nb_etas > 0 and len(nbiter_mcmc) > 1 and nbiter_mcmc[1] > 0:
        domega2 = np.array(varList.get('domega2', np.ones((nb_etas, nb_etas))), dtype=float)
        nrs2 = 1
        for _ in range(nbiter_mcmc[1]):
            ...
            for mk in range(nb_etas):
                ...
                etaMc = etaM.copy()
                etaMc[:, mk] = etaM[:, mk] + np.random.randn(Dargs['NM']) * step
                phiMc[:, varList['ind_eta']] = mean_phiM[:, varList['ind_eta']] + etaMc
                Uc_y, _ = compute_LLy(phiMc, Dargs, varList['pres'])
            ...
            acc_rate = np.divide(accept_counts, total_counts, out=np.zeros_like(accept_counts), where=total_counts > 0)
            domega2[:, nrs2 - 1] = domega2[:, nrs2 - 1] * (1 + opt['stepsize_rw'] * (acc_rate - opt['proba_mcmc']))
    ...
    if nb_etas > 0 and len(nbiter_mcmc) > 2 and nbiter_mcmc[2] > 0:
        ...
        for _ in range(nbiter_mcmc[2]):
            ...
            block = np.arange(nb_etas) if nrs2 >= nb_etas else np.random.choice(nb_etas, size=nrs2, replace=False)
            step = domega2[block, nrs2 - 1]
            etaMc = etaM.copy()
            etaMc[:, block] = etaM[:, block] + np.random.randn(Dargs['NM'], nrs2) * step
            phiMc[:, varList['ind_eta']] = mean_phiM[:, varList['ind_eta']] + etaMc
            Uc_y, _ = compute_LLy(phiMc, Dargs, varList['pres'])
            U_eta = 0.5 * np.sum(etaM * (etaM @ somega), axis=1)
            Uc_eta = 0.5 * np.sum(etaMc * (etaMc @ somega), axis=1)
            ...
            domega2[block, nrs2 - 1] = domega2[block, nrs2 - 1] * (1 + opt['stepsize_rw'] * (acc_rate - opt['proba_mcmc']))
            domega2[block, nrs2 - 1] = np.maximum(domega2[block, nrs2 - 1], 1e-8)
        varList['domega2'] = domega2

    # Kernel 4: MAP-based adaptive proposal
    if nb_etas > 0 and len(nbiter_mcmc) > 3 and nbiter_mcmc[3] > 0 and kiter < opt['nbiter_map']:
        if Dargs.get('modeltype', 'structural') == 'structural':
            ...
            Gamma, chol_Gamma, inv_Gamma = _compute_gamma(gradf0, gradh0, varList, Dargs)
            ...
            for _ in range(nbiter_mcmc[3]):
                ...
                for i in range(Dargs['NM']):
                    g = chol_Gamma[i % N]
                    etaMc[i, :] = eta_map[i, :] + np.random.randn(nb_etas) @ g.T
                phiMc = phiM.copy()
                phiMc[:, varList['ind_eta']] = mean_phiM[:, varList['ind_eta']] + etaMc
                Uc_y, _ = compute_LLy(phiMc, Dargs, varList['pres'])
                Uc_eta = 0.5 * np.sum(etaMc * (etaMc @ somega), axis=1)
...
def _compute_phi_map(mean_phi, Dargs, varList, opt):
    ...
    ytype_norm = _normalize_ytype(ytype[:Dargs['nobs']] if ytype is not None else None, ntypes)
        ...
        if Dargs.get('modeltype', 'structural') == "structural":
            def objective(phi1_opt):
                return conditional_distribution_c(
                    phi1_opt, phii, idi, xi, yi, mean_phi1, i1_omega2, iomega,
                    Dargs['transform_par'], Dargs['structural_model'], varList['pres'],
                    Dargs['error_model'], ytype_norm[mask] if ytype_norm is not None else None
                )
        else:
            def objective(phi1_opt):
                return conditional_distribution_d(
                    phi1_opt, phii, idi, xi, yi, mean_phi1, i1_omega2, iomega,
                    Dargs['transform_par'], Dargs['structural_model']
                )
        try:
