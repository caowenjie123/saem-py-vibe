...
def _estimate_residual(y, f, error_model, ytype, current_pres):
    eps = 1e-10
    error_model = list(error_model) if isinstance(error_model, (list, tuple)) else [error_model]
    ...
    if len(error_model) == 1:
        ...
        if em == 'constant' or em == 'exponential':
            ...
            res = minimize(nll, x0, method='L-BFGS-B', bounds=[(eps, None), (eps, None)])
            pres[:2] = res.x
        return pres
...
def mstep(kiter, Uargs, Dargs, opt, structural_model, phiM, varList, suffStat, mean_phi_init):
    ...
    if stepsize > 0:
        ...
        if nchains <= 1:
            ...
            phi_chain = phiM.reshape((nchains, N, nb_parameters))
        phi_eta = phi_chain[:, :, varList['ind_eta']] if len(varList['ind_eta']) > 0 else np.zeros((nchains, N, 0))
        ...
        for k in range(nchains):
            if len(varList['ind_eta']) > 0:
                phik = phi_eta[k, :, :]
                stat2 += phik.T @ phik
        suffStat['statphi1'] = suffStat['statphi1'] + stepsize * (stat1 / nchains - suffStat['statphi1'])
        suffStat['statphi2'] = suffStat['statphi2'] + stepsize * (stat2 / nchains - suffStat['statphi2'])
        suffStat['statphi3'] = suffStat['statphi3'] + stepsize * (stat3 / nchains - suffStat['statphi3'])
        ...
        if Dargs.get('modeltype', 'structural') == "structural":
            ...
            new_pres = _estimate_residual(Dargs['yM'], fpred, error_model, ytype, varList['pres'])
            if kiter <= opt['nbiter_sa']:
                varList['pres'] = np.maximum(varList['pres'] * opt['alpha1_sa'], new_pres)
            else:
                varList['pres'] = varList['pres'] + stepsize * (new_pres - varList['pres'])
