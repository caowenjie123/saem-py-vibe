...
def initialise_main_algo(
    saemix_data: SaemixData, saemix_model: SaemixModel, saemix_options: dict
):
    ...
    if ncov > 0:
        for idx, orig_idx in enumerate(unique_indices):
            cov_values = (
                saemix_data.data[saemix_data.data["index"] == orig_idx][
                    saemix_data.name_covariates
                ]
                .iloc[0]
                .values
            )
    ...
    betaI_ini = transpsi(
        fixedpsi_ini.reshape(1, -1), saemix_model.transform_par
    ).flatten()
    ...
    indx_betaC = (
        np.where(saemix_model.betaest_model[1:, :].flatten() > 0)[0] + nb_parameters
    )

    indx_fix10 = np.where(
        (saemix_model.fixed_estim > 0) & (np.diag(saemix_model.covariance_model) == 0)
    )[0]
    indx_fix11 = np.where(
        (saemix_model.fixed_estim > 0) & (np.diag(saemix_model.covariance_model) > 0)
    )[0]
    ...
    omega_eta = (
        omega[np.ix_(i1_omega2, i1_omega2)] if len(i1_omega2) > 0 else np.zeros((0, 0))
    )
    diag_omega = np.diag(omega) if omega.size > 0 else np.array([])
    if len(i1_omega2) > 0:
        base = np.sqrt(np.maximum(np.diag(omega_eta), 1e-10)) * saemix_options.get(
            "rw_init", 0.5
        )
    ...
    nbiter_burn = saemix_options.get("nbiter_burn", 0)
    nbiter_tot = nbiter_saemix[0] + nbiter_saemix[1]

    stepsize = np.ones(nbiter_tot)
    if nbiter_saemix[1] > 0:
        stepsize[nbiter_saemix[0] :] = 1.0 / np.arange(1, nbiter_saemix[1] + 1)
    if nbiter_burn > 0:
        stepsize[: min(nbiter_burn, nbiter_tot)] = 0.0
    ...
    etype_exp = [
        i for i, em in enumerate(saemix_model.error_model) if em == "exponential"
    ]
    Dargs = {
        "IdM": IdM,
        "XM": XM,
    ...
    if saemix_model.modeltype == "structural":
        ...
        if tries >= max_tries:
            raise RuntimeError(
                "Failed to find a valid initial parameter guess for structural model."
            )
